                    ...
                    @forelse($titulos as $titulo)
                        <tr>
                            ...
                            <td class="text-center">
                                <!-- Editar -->
                                <a href="{{ route('titulos.edit', $titulo->id) }}" class="btn btn-link btn-xs"
                                   title="Editar">
                                    <span class="glyphicon glyphicon-pencil"></span>
                                </a>
                                <!-- Excluir -->
                                <a href="#" class="btn btn-link btn-xs" title="Excluir" data-toggle="modal"
                                   data-target="#modalExcluirTitulo" data-id="{{ $titulo->id }}"
                                   data-descricao="{{ $titulo->descricao }}">
                                    <span class="glyphicon glyphicon-remove"></span>
                                </a>
                            @if($titulo->status == 'PENDENTE')
                                <!-- Receber -->
                                    <a href="{{ route('titulos.receive', $titulo->id) }}"
                                       class="btn btn-link btn-xs receber-titulo" title="Receber"
                                       data-id="{{ $titulo->id }}" data-token="{{ csrf_token() }}">
